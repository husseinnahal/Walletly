...
import UpdatePay from "./updatePayment"
...
export default function Payments({ visible, onClose, id ,refresh,onaddPay}) {
    ...
    useEffect(() => {
        ...
        const fetchPayments = async () => {
            ...
            try {
                ...
                const response = await axios.get(`https://walletlyapi.onrender.com/api/goals/saved-amount/${id}`, {
    ...
    const accept = useCallback(async (idToDelete) => {
        try {
            const token = Cookies.get("Walletly_Token");
            await axios.delete(`https://walletlyapi.onrender.com/api/goals/saved-amount/${id}/${idToDelete}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
    ...
    return (
        <>
            ...
            <Toast ref={toast} />
            <UpdatePay  visible={upPay} onClose={() => setUppay(false)} goalId={id} pay={pays} handlePay={()=>setHandelDel((up)=>!up)} handleGoals={refresh}/>
